      ...
      <div style={{
        ...
        <div style={{ padding: '28px 28px 32px' }}>
          ...
          <p style={{ fontSize: 12, color: '#475569', margin: '0 0 20px' }}>
            Sign in to access all sections
          </p>

          {/* Tab toggle */}
          <div style={{
            display: 'flex', background: '#0f1b2e', borderRadius: 8,
            border: '1px solid #1a2d4a', padding: 3, marginBottom: 20,
          }}>
            {['login', 'signup'].map(m => (
          ...
          <form onSubmit={handle}>
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 4 }}>Email</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="you@example.com"
                style={{ width: '100%', padding: '9px 12px', borderRadius: 8, boxSizing: 'border-box', background: '#0f1b2e', border: '1px solid #1a2d4a', color: '#f1f5f9', fontSize: 13, outline: 'none' }}
                onFocus={e => e.target.style.borderColor = '#FF6B00'}
                onBlur={e => e.target.style.borderColor = '#1a2d4a'}
              />
            </div>

            <div style={{ marginBottom: mode === 'signup' ? 10 : 18 }}>
              <label style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 4 }}>Password</label>
            ...
            {mode === 'signup' && (
              <div style={{ marginBottom: 18 }}>
                <label style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 4 }}>Confirm Password</label>
            ...
            {error && <div style={{ marginBottom: 10, padding: '8px 12px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 7, fontSize: 12, color: '#f87171' }}>{error}</div>}
            {msg   && <div style={{ marginBottom: 10, padding: '8px 12px', background: 'rgba(34,197,94,0.1)',  border: '1px solid rgba(34,197,94,0.3)',  borderRadius: 7, fontSize: 12, color: '#4ade80' }}>{msg}</div>}

            <button type="submit" disabled={busy} style={{
              width: '100%', padding: '10px 0', borderRadius: 8, border: 'none',
              background: busy ? '#7c3a1a' : '#FF6B00', color: '#fff',
              fontSize: 14, fontWeight: 700, cursor: busy ? 'not-allowed' : 'pointer', transition: 'background 0.2s',
            }}>
